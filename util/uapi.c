...
{
    if (VDEBUG) printf("Entering api:generate\n");
    ...
    int ret = 0;
    if (VDEBUG) printf("\tapi:generate::variables_initialized\n");
    
    ret = crypto_sign_keypair(pk, sk);

    if (VDEBUG) printf("\tapi:generate::keys_generated\n");
    if (ret != 0)
    {
        return -1;
    }
    if (VDEBUG) printf("\tapi:generate::shaking\n");
...
{
    if (VDEBUG) printf("Entering api:sign\n");
    size_t siglen; 
